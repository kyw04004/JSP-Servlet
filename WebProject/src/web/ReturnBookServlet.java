...
public class ReturnBookServlet extends HttpServlet {
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("ReturnBookView.jsp");
	...
	private BookList readDB(int upperSeqNo)
		...
		ResultSet rs = stmt.executeQuery("select * from jsplibrary where borrow like 'N' order by id asc;"); 
		int cnt=0;
		while(true) {
			if(!rs.next())
				break;
